...
**Targeting.** Local mode: `--dir <path>` or `$WIKID_DIR`. Direct remote mode: `--server <url>` + optional `--token <t>` + required `--wiki <name>`, or their `$WIKID_SERVER`/`$WIKID_TOKEN`/`$WIKID_WIKI` equivalents. Named remote profiles live in `[remotes.<name>]` with required `server`, optional `token`, and optional daemon `wiki` (defaults to the profile name). Explicit `--dir` and `--server` flags win over env. `--wiki <name>` without an explicit `--server` is always a unified config selector across local `[wikis]` and client-only `[remotes]`, and intentionally ignores `$WIKID_SERVER`; overriding `$WIKID_WIKI` on an env-selected daemon now requires also passing `--server`. This is a deliberate narrowing from the former field-by-field flag/env composition so a profile name cannot accidentally retarget an ambient server. `--token` may override a selected profile token but is not a target by itself. Env-only local+remote targets → usage error.

Config discovery is `--config`, `$WIKID_CONFIG`, `./wikid.toml`, then `~/.config/wikid/config.toml`. Without an explicit selector, resolution chooses the local wiki whose canonical path contains cwd (longest prefix), else the only target across local wikis and remotes, else `default_wiki` across that unified namespace. Cwd-prefix intentionally beats a remote default: operators enforcing daemon-only writes must not register the same remote vault as a local `[wikis]` entry. A name present in both tables or multiple targets with no valid default returns `ambiguous_wiki`; an explicit unknown name returns `unknown_wiki` with available names. `default_remote` is deliberately rejected: one `default_wiki` avoids dual-default ambiguity. For a selected remote, token precedence is explicit `--token`, profile token, then `$WIKID_TOKEN` only when the profile has none. On Unix, discovering a group/world-accessible config containing `[tokens]` or a remote token prints a `chmod 600` warning to stderr without changing the exit code or corrupting JSON stdout. Token-bearing profiles belong in the global 0600 config, not a repo-local `./wikid.toml` that could be committed. Remote mode maps commands 1:1 onto the HTTP API and renders identically. HTTP error bodies re-render verbatim; connection/decode failures use `transport`, unrecognized error bodies `http`. ProxyCommand/transport hooks are deferred.
...
- `config list` — read-only client-side inventory of the discovered config. It prints the config path, daemon bind, optional default, and every local/remote target without token values. Remote entries resolve an omitted daemon `wiki` to the profile name. JSON shape: `{config_path, bind, default_wiki?, targets:[{kind:"local",name,path}|{kind:"remote",name,server,wiki}]}`. This command does not select or contact a wiki.
