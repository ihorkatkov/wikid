...
**Targeting.** Local mode: `--dir <path>` or `$WIKID_DIR`. Direct remote mode: `--server <url>` + optional `--token <t>` + required daemon `--wiki <name>`, or their `$WIKID_SERVER`/`$WIKID_TOKEN`/`$WIKID_WIKI` equivalents. Named remote profiles live in `[remotes.<name>]` with required `server`, optional `token`, and optional daemon `wiki` (defaults to the profile name). `--target <name>` is the preferred unified config selector across local `[wikis]` and client-only `[remotes]`; it conflicts with `--dir`, `--server`, and `--wiki`. For compatibility, `--wiki <name>` without an explicit `--server` remains an alias for `--target` and intentionally ignores `$WIKID_SERVER`; with explicit `--server`, `--wiki` names the daemon wiki. Explicit target flags win over opposite-mode env. `--token` may override a selected profile token but is not a target by itself. Env-only local+remote targets → usage error.

Config discovery is `--config`, `$WIKID_CONFIG`, `./wikid.toml`, then `~/.config/wikid/config.toml`. Without an explicit selector, resolution chooses the local wiki whose canonical path contains cwd (longest prefix), else the only target across local wikis and remotes, else `default_wiki` across that unified target namespace. Cwd-prefix intentionally beats a remote default. A name present in both tables or multiple targets with no valid default returns `ambiguous_wiki`; an explicit unknown name returns `unknown_wiki` with available names. For a selected remote, token precedence is explicit `--token`, profile token, then `$WIKID_TOKEN` only when the profile has none. On Unix, discovering a group/world-accessible config containing tokens prints a `chmod 600` warning to stderr without corrupting JSON stdout. Remote mode maps operations 1:1 onto HTTP while the human client view adds target provenance and target-preserving hints. HTTP error bodies re-render verbatim; connection/decode failures use `transport`, unrecognized error bodies `http`. ProxyCommand/transport hooks are deferred.

**Output.** Bare `wikid` is an orientation dashboard: a token-safe list of every configured target (active/default marked), then active status, then exactly four hints headed by unconditional `wikid skills get core`. Bare `wikid --json` emits `{targets:[...],active_target,status}`. Explicit `status` is focused: local human output includes actionable root; remote human output identifies target, daemon wiki, server URL, and client/server versions while omitting the non-actionable server root. `status --json` remains the unmodified `VaultStatus` wire struct. All successful human operation hints preserve explicit/config targeting and never print token values. Human `cat`/`cat --hashes` inserts a single `---` between content and client metadata/hints, so page-authored lines resembling `hint:` remain byte-faithful and cannot be mistaken for generated output; the last separator is authoritative when page content itself contains `---`, and JSON is unchanged. Errors are stdout/exit 1; usage errors are clap exit 2.

**AXI conformance checklist** (each item is a test):
1. Bare `wikid` = live orientation dashboard (all targets + active status), never help text; explicit `status` is focused.
...
7. Focused human commands end with 1–2 target-preserving `hint:` lines. The bare dashboard ends with four: skills onboarding, switch target, search, doctor. No hints in JSON.
...
- `config list` — read-only client-side inventory of the discovered config. It prints the config path, daemon bind, optional default, and every local/remote target without token values. Remote entries resolve an omitted daemon `wiki` to the profile name. JSON shape: `{config_path, bind, default_wiki?, targets:[{kind:"local",name,path}|{kind:"remote",name,server,wiki}]}`. Its next-step hint uses `--target`. This command does not select or contact a wiki.
...
- bare `wikid` — orientation dashboard; JSON shape `{targets:[{kind,name,path?|server?,wiki?,active,default,configured}],active_target,status}`
- `status` — focused active-target status; JSON remains `VaultStatus`
...
- Routes (all under a named wiki; unknown wiki → 404 `unknown_wiki` listing available names). In remote mode, `status.root` is the server-side filesystem path; the human CLI omits it as non-actionable, while explicit `status --json` preserves the shared `VaultStatus` struct unchanged:
...
- Config (TOML, see `wikid-server::config`): `bind` (default `127.0.0.1:7448`), optional unified `default_wiki`, `[wikis] name = "/path"`, `[tokens] "actual_secret" = "actor-name"`, and client-only `[remotes.<name>]` profiles with `server`, optional `token`, and optional daemon `wiki`. The server ignores but preserves `remotes`. Actor names are logged (`tracing`) but not otherwise used in MVP. Config rewrites preserve keys/values (comments may be lost), use temp+rename where feasible, and set Unix mode `0600`.
